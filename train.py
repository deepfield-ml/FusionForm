...
def recommend_formation_onnx(opponent_formation, ort_session, le, num_classes):
    ...
    for our_idx in range(num_classes):
        ...
        print(f"Evaluating formation {le.inverse_transform([our_idx])[0]}: score = {score}")  # Debug statement
        
        if score > best_score:
            best_score = score
            best_formation = le.inverse_transform([our_idx])[0]
    
    print(f"Best formation: {best_formation} with score {best_score}")  # Debug statement
